...
import { buildContext, readForContext, loadBrief, VERIFIED_OBEDIENT_TOKENS } from '../scripts/council/context.mjs';
import { createEmitter, reduce, redactLine, SCHEMA } from '../scripts/council/events.mjs';
import { createRenderer } from '../scripts/council/render.mjs';
import { up, clearBelow } from '../scripts/council/ansi.mjs';
...
console.log('\n\u25b8 Convening \u2014 a missing CLI is reported and stepped over, never retried');
{
  ...
  const roster = path.join(ROOT, 'scripts', 'council', 'members.json');
...
{
  const cfg = JSON.parse(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'members.json'), 'utf8'));
  ...
  check('containment was measured rather than declared',
    ...
    'written by scripts/council/verify-containment.mjs');
...
console.log('\n▸ Rendering — the same reducer drives the terminal and an external watcher');
{
  ...
  const w = spawnSync('node', [path.join(ROOT, 'scripts', 'council', 'watch.mjs'), f, '--once'],
  ...
  const none = spawnSync('node', [path.join(ROOT, 'scripts', 'council', 'watch.mjs'), path.join(dir, 'nope.ndjson')],
    { encoding: 'utf8', timeout: 20_000 });
...
console.log('\n▸ The council graded itself at 5.0/10 — these are what it found');
{
  ...
  {
    const src = fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'context.mjs'), 'utf8');
...
// WAS OPEN, and it punished the members with the best instincts. The probe read
  ...
  // resolving the link and then comparing against the resolved link.
  {
    ...
    try {
      ...
      check('...and extra roots must come from the operator, not the repo',
        /COUNCIL_EXTRA_ROOTS/.test(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'context.mjs'), 'utf8')));
    } finally { fs.rmSync(root, { recursive: true, force: true }); }
  }
  ...
  // failure with no indication why.
  {
    ...
    check('...and council.mjs substitutes it',
      /\{timeoutMin\}/.test(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'utf8')));
  }
...
console.log('\n▸ Round three — 7.0/10, and two of these were regressions from round two');
{
  ...
  check('...and the run file says so out loud',
    /This tally carries no information/.test(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'utf8')));
...
console.log('\n▸ Round four — 7.0/10 (range 5.5–7.8), and one finding was self-inflicted');
{
  ...
  check('the pre-spend argv estimate is measured in bytes',
    /Buffer\.byteLength\(preamble, 'utf8'\)/.test(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'utf8')),
  ...
  // it could speak and reported as "timed out after 0 min".
  {
    ...
    const run = (t) => spawnSync('node', [path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'q', '--preflight', `--timeout=${t}`],
      { encoding: 'utf8', timeout: 40_000, cwd: dir });
  ...
  check('only a LEADING tilde is expanded in scratchDir',
    /replace\(\/\^~\(\?=\\\/\|\$\)\/, os\.homedir\(\)\)/.test(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'utf8')));

  // WAS OPEN: the write boundary was lstat-then-open, a race no amount of checking can close.
  {
    const src = fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'safe-write.mjs'), 'utf8');
  ...
  check('only successful reviews reach the tally',
    /borda\(reviews\.filter\(\(r\) => r\.ok\), ids\)/.test(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'utf8')));
  ...
  check('the report discloses when overlap used a subset of members',
    /Overlap basis/.test(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'utf8')));
}
...
console.log('\n▸ Round five — 7.3/10, and the entry point was the last silent-wrong-answer path');
{
  ...
  check('the event stream is opened with O_NOFOLLOW too',
    /O_NOFOLLOW/.test(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'events.mjs'), 'utf8')));
  ...
  {
    const allow = fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'utf8')
  ...
  check('relative PATH entries are skipped rather than resolved differently',
    /path\.isAbsolute\(d\)/.test(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'utf8')));
  ...
  check('a failed JSON write is not then reported as written',
    /if \(jsonWritten\.ok\) log/.test(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'utf8')));
  ...
  check('a slug collision below the truncation limit is detected',
    /if \(prior\.question && prior\.question !== question\)/.test(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'utf8')));
...
console.log('\n▸ Round six — 7.3/10, and one line closed a silent-corruption class');
{
  ...
  const councilSrc = fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'utf8');
  ...
  // WAS OPEN: --local-roster with no such file silently used the packaged roster.
  {
    ...
    const r = spawnSync('node', [path.join(ROOT, 'scripts', 'council', 'council.mjs'), 'q', '--preflight', '--local-roster'],
      { encoding: 'utf8', timeout: 40_000, cwd: dir });
  ...
  check('the write boundary states precisely what O_NOFOLLOW does NOT cover',
    /a PARENT directory swapped for a symlink/.test(fs.readFileSync(path.join(ROOT, 'scripts', 'council', 'safe-write.mjs'), 'utf8')));
