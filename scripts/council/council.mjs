...
import { buildContext, BRIEF } from './context.mjs';
import { judgeOutput } from './judge-output.mjs';

const HERE = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.dirname(path.dirname(HERE));
const CFG = JSON.parse(fs.readFileSync(path.join(HERE, 'members.json'), 'utf8'));
const OUT_DIR = path.join(ROOT, 'docs', 'council');
...
const preamble = [BRIEF, ctx.text].filter(Boolean).join('\n\n---\n\n');
...
const md = [
  ...
  ...(tally ? [
    ...
    `agreeing is weak evidence; this project keeps 77 disproven claims that were mostly unanimous.`,
