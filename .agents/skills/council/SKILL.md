...
watch it directly can run `node "$CLAUDE_PLUGIN_ROOT/scripts/council/watch.mjs"` in another terminal — tell
...
node "$CLAUDE_PLUGIN_ROOT/scripts/council/council.mjs" "<question>" --context <file>... --detach
...
node "$CLAUDE_PLUGIN_ROOT/scripts/council/feed.mjs" --every=30

# 3. Or just ask, any time, from any session. Cheap, exits immediately.
node "$CLAUDE_PLUGIN_ROOT/scripts/council/status.mjs"
```
