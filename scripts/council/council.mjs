...
//
//   node scripts/council/council.mjs "<question>" [--context file ...]
...
//
//   node scripts/council/verify-containment.mjs        prove no member can write. Exit 3 if one can.
//   node scripts/council/watch.mjs                    follow a run's event stream from anywhere
...
// ── arguments ────────────────────────────────────────────────────────────────
//
// Parsed ONCE, against a declared schema, in scripts/council/args.mjs. It used to be three ad-hoc helpers
...
if (!parsed.ok) {
  ...
  if (parsed.hint === 'unknown') {
    ...
    console.error('\n      node scripts/council/council.mjs "Should we use --lenses here?" --context src/x.js');
    console.error('\n  Or put the question after `--`, and nothing in it is read as a flag:');
    console.error('\n      node scripts/council/council.mjs -- Should we use --lenses here?');
  }
...
if (questionTokens.length > 1 && anyFlag && !argv.includes('--')) {
  ...
  console.error('\n      node scripts/council/council.mjs "the whole question here" --context src/x.js');
  console.error('\n  Or pass it after `--`:  node scripts/council/council.mjs -- the whole question here\n');
  process.exit(1);
}
...
if (notFiles.length) {
  ...
  console.error('\n      node scripts/council/council.mjs "the question" --context src/a.js src/b.js\n');
  process.exit(1);
}
...
// path, and the pid. `scripts/council/status.mjs` turns those into an answer.
if (has('detach')) {
...
// permission. See scripts/council/verify-containment.mjs for the measurement and the two probes.
//
...
if (uncontained.length) {
  if (allowUncontained) {
    ...
    for (const m of uncontained) log(`    🚨 ${m.label} (verify with: node scripts/council/verify-containment.mjs --members=${m.id})`);
...
if (!members.length) {
  ...
  if (!requested.length && uncontained.length) {
    ...
    log(`\n  Either run \`node scripts/council/verify-containment.mjs\` to re-check them, or pass`);
