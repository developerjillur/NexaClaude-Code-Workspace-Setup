...
import { buildContext, readForContext } from '../scripts/council/context.mjs';
import { judgeOutput } from '../scripts/council/judge-output.mjs';
...
console.log('\n▸ Context containment — a denylist cannot be completed, so confine instead');
{
  ...
  const link = path.join(ROOT, 'docs', 'council-test-notes.md');
  ...
  check('refuses .env by path', refused('code/.env'));
  check('refuses data/', refused('code/data/anything.db'));
  check('refuses the prompt log', refused('docs/prompts/2026-07-28.md'));
...
{
  const f = path.join(ROOT, 'docs', 'council-test-secret.md');
...
{
  const f = path.join(ROOT, 'docs', 'council-test-big.md');
  ...
  const a = path.join(ROOT, 'docs', 'council-test-a.md');
  const b = path.join(ROOT, 'docs', 'council-test-b.md');
  fs.writeFileSync(a, 'a'.repeat(79_000));
  fs.writeFileSync(b, 'b'.repeat(79_000));
  const c = path.join(ROOT, 'docs', 'council-test-c.md');
  fs.writeFileSync(c, 'c'.repeat(79_000));
  const pack = buildContext(['docs/council-test-a.md', 'docs/council-test-b.md', 'docs/council-test-c.md'], ROOT);
...
{
  const pack = buildContext(['AGENTS.md'], ROOT).text;
  ...
  check('closes the data block after the content', pack.indexOf('End of quoted data') > pack.indexOf('AGENTS.md'));
}
